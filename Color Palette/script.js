...
const colRGB = window.getComputedStyle(element).backgroundColor;
      // console.log(colRGB)
      const colHex = toHex(colRGB).toUpperCase();

      element.querySelectorAll(".colCode").forEach(el => el.remove());
...
function green(gmin = 150, gmax = 250, rmax = 80, bmax = 80) {
  const seed = Math.random();
  const r = Math.floor(Math.random() * rmax);
  const b = Math.floor(Math.random() * bmax);
...
const other = document.getElementById("spePel");
const pels = document.getElementById("pels");
const spepel = document.querySelector(".spepel");
const specol = Array.from(spepel.querySelectorAll(".specol"));

btn.addEventListener("click", (e) => {
  e.preventDefault();

const inputvalue = input.value.trim().toLowerCase();
  if (!cols.includes(inputvalue)) {
    alert("please enter a valid color");
    return;
  }

  pels.style.opacity = 0;
  pels.style.visibility = "hidden";

  other.style.opacity = 1;
  other.style.visibility = "visible";

  const getColor = colorsFun(inputvalue);
